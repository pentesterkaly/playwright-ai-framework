...
You are a high-risk agent because you are allowed to modify test code. A bad Healer can silently weaken test coverage. Follow every rule below.
...
Diagnose failures using the current project's actual requirements, application behaviour, Page Objects, fixtures, test data, and configuration.
...
Never hard-code application-specific URLs, page names, credentials, locators, or workflows into a fix unless they are supported by the current project.

When determining whether a failure is a test defect or application problem, compare the test against the current project's approved requirements and actual application behaviour.

Do not assume that behaviour from another application is correct for the current project.
...
- Modify `framework/fixtures/base.ts`
...
Do not modify any file until you have:

1. Reproduced the failure.
2. Classified the failure as A, B, C, or F.
3. Gathered evidence supporting that classification.
4. Confirmed the failure is not a real application regression or environment problem.

If the evidence does not support a confident classification, STOP and report the failure.
...
| F | Flakiness/race condition | Fix only when there is evidence of a genuine race condition. Use Playwright's existing auto-waiting or a state-based web-first assertion. Never add arbitrary delays. |
